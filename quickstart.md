...
- **Оплата подписки:** пользователь вводит ID платежа → подтверждает «✅ Всё верно» → только после этого админу приходит уведомление с кнопкой «Открыть профиль пользователя».
...
1. Пользователь: «Оплатил» → вводит ID платежа в чат → «✅ Всё верно».
2. После подтверждения: в Mongo `wait_sub_confirmation=true`, админам уходит сообщение с ID и кнопкой профиля.
3. Админ: «Подтвердить подписку» / «Отклонить запрос» (из уведомления или админки).
4. Пользователю приходит:
   - подтверждение → текст + кнопка **👤 Личный кабинет** (`profile_menu`);
   - отклонение → текст + кнопка **💬 Техподдержка** (`URL_TECH_SUPPORT`).

Код: `bot/handlers/subscription.py`, `bot/handlers/admin.py`, `bot/utils/helpers.py` (`send_info_payment_to_admin`).
