...
1. «Приобрести подписку» → оплата на кошелёк (`URL_PAYMENT`, TRC-20).
2. «Оплатил» → ввод ID платежа в чат.
3. Подтверждение «✅ Всё верно» — только после этого заявка считается отправленной.

До шага 3 админ **не** получает уведомление.
...
Клавиатура: `bot/keyboards/inline_kb.py` → `payment_status_notify_kb`. Отправка: `bot/handlers/admin.py` → `_notify_user_payment_status`.
