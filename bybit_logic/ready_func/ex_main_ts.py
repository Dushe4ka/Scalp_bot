...
С трейлинг стоп-лоссом, который активируется сразу после открытия позиции
"""
...
def handle_ticker_price(message):
    ...
    4. Обновляет трейлинг стоп (если он активен)
    5. Выводит информацию в лог каждые N секунд
    """
    global entry_price, position_qty, trailing_stop
...
def main():
    ...
    logger.info(f"📊 Трейлинг стоп: будет обновляться каждые {TRIGGER_TS_PERCENTAGE}%")
    ...
    # ============================================
    # ШАГ 6.2: АКТИВАЦИЯ ТРЕЙЛИНГ СТОПА СРАЗУ ПОСЛЕ ОТКРЫТИЯ ПОЗИЦИИ
    # ============================================
    logger.info("🟢 Активирую трейлинг стоп сразу после открытия позиции...")
    
    # Получаем текущую цену для активации трейлинг стопа
    tickers = market.get_tickers_by_symbol(http_session, SYMBOL)
    current_market_price = float(tickers['result']['list'][0]['lastPrice'])
    
    # Активируем трейлинг стоп на текущей цене
    if trailing_stop is not None:
        trailing_stop.activate(current_market_price)
        logger.info(f"✅ Трейлинг стоп активирован на цене {current_market_price:.8g}")
        logger.info(f"📊 Трейлинг стоп будет обновляться каждые {TRIGGER_TS_PERCENTAGE}% роста цены")
    else:
        logger.error("❌ Объект трейлинг стопа не создан!")
        return

    # ============================================
    # ШАГ 6.3: УСТАНОВКА СТОП-ЛОССА
    # ============================================
    ...
    logger.info(f"⏳ Трейлинг стоп активен и будет обновляться каждые {TRIGGER_TS_PERCENTAGE}%...")
